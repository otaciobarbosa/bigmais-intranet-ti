...
do
	...
	cd "$3"
	cnpj=`grep -o "<[dD][eE][sS][tT]><[cC][nN][Pp][jJ]>[0-9]\+</[cC][nN][Pp][jJ]>" $i | grep -o "[0-9]\+"`
	data=`grep -o "<[dD][eE][mM][iI]>[0-9-]\+</[dD][eE][mM][iI]>" $i | sed -e "s/[^0-9]//g"`
	mkdir -p ${cnpj}/${data}
...
cd  /u/rede/xml/
chmod 777 -R xml-txt xml-imp > /tmp/gerarlote.log
