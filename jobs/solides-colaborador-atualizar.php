...
$limpaLog = $conexao->query('TRUNCATE TABLE ZMDSLDINTEGRACAO');
...
try {
    $funcionariosSemCodigo = $conexao->query('SELECT * FROM OBVSLDFUNC');
    ...
    while ($rowFuncionariosAtualizar = $funcionariosSemCodigo->fetch(PDO::FETCH_ASSOC)) {
        ...
        $senior = ''; // mb_convert_encoding(addslashes($rowFuncionariosAtualizar["senior"]), "UTF-8", "ISO-8859-1");
      ...
      $jsonEnviadoTXT = fopen("logsTXT/". $name . " jsonEnviado.txt","a");
      fwrite($jsonEnviadoTXT,''. $jsonEnviado .'');
      fclose($jsonEnviadoTXT);
        ...
        $responseTXT = fopen("logsTXT/". $name . " response.txt","a");
        fwrite($responseTXT, $response . ";");
        fclose($responseTXT);
    ...
    if($httpCode <> 200){
      ...
      $resp = $httpCode;
        ...
        $stmt->bindParam(':json', $jsonEnviado);

        if ($stmt->execute()) {
          // echo "Registro inserido com sucesso!";
      } else {
          // Registre o erro, mas continue com o próximo registro
          $errorInfo = $stmt->errorInfo();
          $errorMessage = "Erro na consulta SQL: " . $errorInfo[2];
